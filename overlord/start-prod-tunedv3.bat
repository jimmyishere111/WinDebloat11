...
REM Tuned production launcher v2 GÇö with watchdog
REM Usage:
REM   scripts\start-prod-tunedv2.bat
REM   scripts\start-prod-tunedv2.bat loadtest
REM   scripts\start-prod-tunedv2.bat loadtest-baseline
REM   scripts\start-prod-tunedv2.bat loadtest-profile
REM
...
REM   OVERLORD_CLIENT_PRESENCE_SYNC_MS=60000
REM   OVERLORD_VIEWER_BACKPRESSURE_BYTES=2097152
REM   OVERLORD_MEDIA_VIEWER_BACKPRESSURE_BYTES=524288
...
if not defined OVERLORD_CLIENT_DB_SYNC_MS set OVERLORD_CLIENT_DB_SYNC_MS=30000
if not defined OVERLORD_CLIENT_DB_SYNC_BATCH_SIZE set OVERLORD_CLIENT_DB_SYNC_BATCH_SIZE=100
if not defined OVERLORD_CLIENT_PRESENCE_SYNC_MS set OVERLORD_CLIENT_PRESENCE_SYNC_MS=60000
if not defined OVERLORD_VIEWER_BACKPRESSURE_BYTES set OVERLORD_VIEWER_BACKPRESSURE_BYTES=2097152
if not defined OVERLORD_MEDIA_VIEWER_BACKPRESSURE_BYTES set OVERLORD_MEDIA_VIEWER_BACKPRESSURE_BYTES=524288
...
if not defined OVERLORD_OFFLINE_BATCH_DELAY_MS set OVERLORD_OFFLINE_BATCH_DELAY_MS=100
...
REM TLS handled by Overlord itself (nginx stream proxy, not HTTP)
if not defined OVERLORD_TLS_OFFLOAD set OVERLORD_TLS_OFFLOAD=true
if not defined OVERLORD_TRUST_PROXY set OVERLORD_TRUST_PROXY=true
...
if /I "%MODE%"=="loadtest" (
  if not defined OVERLORD_AGENT_TOKEN (
    ...
    echo   scripts\start-prod-tunedv2.bat loadtest
    endlocal
    exit /b 1
  )
  ...
  if not defined OVERLORD_AGENT_TOKEN (
    ...
    echo Example:
    echo   set OVERLORD_AGENT_TOKEN=some-long-random-token
    echo   scripts\start-prod-tunedv2.bat loadtest-profile
    endlocal
    exit /b 1
  )
  ...
  if not defined OVERLORD_AGENT_TOKEN (
    ...
    echo Example:
    echo   set OVERLORD_AGENT_TOKEN=some-long-random-token
    echo   scripts\start-prod-tunedv2.bat loadtest-baseline
    endlocal
    exit /b 1
  )
  set OVERLORD_LOAD_TEST_ALLOW_UNSIGNED_AGENTS=true
...
echo === Overlord tuned production server v2 (watchdog) ===
echo HOST=%HOST%
echo PORT=%PORT%
...
echo OVERLORD_CLIENT_PRESENCE_SYNC_MS=%OVERLORD_CLIENT_PRESENCE_SYNC_MS%
echo OVERLORD_VIEWER_BACKPRESSURE_BYTES=%OVERLORD_VIEWER_BACKPRESSURE_BYTES%
echo OVERLORD_MEDIA_VIEWER_BACKPRESSURE_BYTES=%OVERLORD_MEDIA_VIEWER_BACKPRESSURE_BYTES%
...
REM v2: Watchdog loop GÇö auto-restart if server hangs or crashes
:watchdog
echo [watchdog] Starting server (watchdog active)...
if /I "%OVERLORD_PROFILE_SOURCE_MODE%"=="true" (
  call bun src/index.ts
) else (
  call "%ROOT%Overlord-Server\dist\overlord-server.exe"
)
echo [watchdog] Server stopped or hung. Restarting in 3 seconds...
timeout /t 3 >nul
goto watchdog
